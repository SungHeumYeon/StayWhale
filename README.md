...
- [MVC2 모델 구조에서 Spring FrameWork 구조로 변경](https://github.com/webpofol/ReviewBulletin)
- File Multiple 속성을 이용한 다수의 후기 사진 업로드 가능
- 8개 게시글 출력 후 페이징 처리
- 제목, 작성자, 호텔&리조트 숙소명 기준 검색 기능
#### 숙소 예약
- 후기의 갯수 기준으로 추천 순 정렬, JsonArray를 활용한 낮은가격 순 정렬, 높은 가격 순 정렬
- 카카오맵API를 활용한 내 위치 및 호텔 리스트 위치 출력 후 선택 시 예약Page로 이동
- 다수의 CheckBox를 활용한 검색 기능 강화
- DateRangePicker를 활용한 달력 날짜 기준 예약 가능한 객실 조회
